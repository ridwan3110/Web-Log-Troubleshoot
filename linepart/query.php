...
if(empty($napro)){
    ...
    if(empty($napra)){
          $namaErr2 = "Part masih kosong<br>";
        }
        if(empty($thn)){
          $namaErr3 = "Tahun masih kosong<br>";
        }
$jan=mysqli_fetch_array(mysqli_query($koneksi,"SELECT COUNT(*) AS CURDATES FROM trouble_et INNER JOIN projek
ON (trouble_et.No_Projek=projek.No_Projek)  join perangkat_vm on (perangkat_vm.No_PVM=trouble_et.No_PVM) WHERE projek.nama_projek='$napro' and perangkat_vm.Nama_Perangkat='$napra'
