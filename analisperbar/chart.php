...
//if ($status=$_POST['status']=='tidak pergantian') {
  // code...
  $nama=mysqli_query($koneksi,"SELECT perangkat_vm.Nama_Perangkat as PART
  ...
  AND (RefNumber ='' or RefNumber is null)
...
//}elseif ($status=$_POST['status']=='pergantian') {
  // code...
//}{
  $nama1=mysqli_query($koneksi,"SELECT perangkat_vm.Nama_Perangkat as PART
  FROM trouble_et INNER JOIN penyebab ON (trouble_et.No_Penyebab=penyebab.No_Penyebab)
  JOIN perangkat_vm ON (trouble_et.No_PVM=perangkat_vm.No_PVM) JOIN projek ON (trouble_et.No_Projek=projek.No_Projek)
  WHERE projek.Nama_Projek='$napro'
  AND trouble_et.Tanggal_Done>='$tgl1'
  ...
  $count1=mysqli_query($koneksi,"SELECT count(*) as jumlah
  FROM trouble_et INNER JOIN penyebab ON (trouble_et.No_Penyebab=penyebab.No_Penyebab)
  JOIN perangkat_vm ON (trouble_et.No_PVM=perangkat_vm.No_PVM) JOIN projek ON (trouble_et.No_Projek=projek.No_Projek)
  WHERE projek.Nama_Projek='$napro'
  AND trouble_et.Tanggal_Done>='$tgl1'
...
//}
  ...
  new Chart(document.getElementById("bar-chart-horizontalz"), {
    type: 'horizontalBar',
    data: {
      labels: [<?php while ($b = mysqli_fetch_array($nama1)) { echo '"' . $b[0] . '",';}?>],
      datasets: [
        {
          label: "Total",
